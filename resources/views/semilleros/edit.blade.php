    ...
    <div class="max-w-4xl mx-auto p-6">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Editar Semillero</h1>
                    <p class="text-gray-600 dark:text-gray-400 mt-2">Actualiza la información del semillero</p>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
            <form action="{{ route('semilleros.update', $semillero) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" 
                           id="nombre" 
                           name="nombre" 
                           value="{{ old('nombre', $semillero->nombre) }}"
                ...
                <div>
                    ...
                    <select id="responsable_id" 
                        ...
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('responsable_id', $semillero->responsable_id) == $user->id ? 'selected' : '' }}>
                                {{ $user->name }} ({{ $user->rol }})
                            </option>
                        @endforeach
                    </select>
